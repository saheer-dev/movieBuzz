...
export default function Today() {
  ...
  useEffect(() => {
    axios.get(
        "https://api.themoviedb.org/3/tv/airing_today?api_key=efb536b72f026a1ee09ebca3b1a7abff"
      )
      .then((res) => {
  ...
  return (
    ...
    <div className=" w-screen p-16">
      ...
      <h1 className="text-4xl text-slate-100 mb-4">Today Arriving</h1>{" "}
        ...
        {Today.map((to) => (
          <div className="relative mx-3 aspect-video  rounded-xl overflow-hidden shadow-md shadow-[#f6f7ff3c] mb-4">
            <img 
              ...
              src={`https://image.tmdb.org/t/p/original/${to.backdrop_path}`}
            />


             <div className="absolute inset-0 p-3 flex items-end bg-gradient-to-t from-black via-transparent" >
              <h2 className="text-slate-300 text-xl">{to.original_name}</h2>
            </div>
